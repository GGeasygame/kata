...
pub fn for_unknown_words<F>(words: &Vec<String>, dictionary: &HashSet<String>, mut func: F) -> Vec<String>
    where
        F: FnMut(&str) -> String,
{
    words.iter().map(|word| {
...
mod tests {
    ...
    #[test]
    fn test_for_unknown_words() {
        let expected = vec!["marry*", "had", "a", "little*", "lamb"];
        let words = &vec!["marry".to_string(), "had".to_string(), "a".to_string(), "little".to_string(), "lamb".to_string()];
        let dictionary: &HashSet<_> = &vec!["had".to_string(), "a".to_string(), "lamb".to_string()].into_iter().collect();
        assert_eq!(expected, for_unknown_words(words, dictionary, |word| {format!("{}*", word)}))
    }
}